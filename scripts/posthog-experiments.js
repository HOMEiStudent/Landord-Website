 ...
 *
 * Falls back to local random assignment when PostHog is blocked
...
(function () {
  ...
  var copyMap = {
    'control': {
      ...
      'features-subhead': 'Inspections, maintenance tracking, Renters\' Rights Act compliance tools and tenant communication, managed from your browser.',
    ...
    'variant-a': {
      ...
      'hero-subhead': 'A <strong>tenant portal</strong> and landlord dashboard that handles <strong>property inspections</strong>, maintenance tracking and tenant communication in your browser. Built for UK <strong>estate agents</strong>, letting agents and private landlords. Connected to <a href="https://homeistudent.com">HOMEi, the Shared Household Organiser</a>. We\'re inviting <strong>50 landlords to pilot it free for 1 year</strong>.',
      'features-h2': 'Your tenant portal, property inspections and maintenance tracker in one dashboard',
      'features-subhead': 'A <strong>landlord app</strong> with digital property inspection checklists, tenant maintenance reporting, direct messaging and full portfolio visibility.',
    ...
    'variant-b': {
      ...
      'features-subhead': 'Track <strong>periodic tenancy</strong> deadlines, build <strong>Section 8 evidence packs</strong>, manage maintenance with timestamped audit trails and communicate with tenants from your browser.',
  ...
  function getLocalVariant() {
    ...
    for (var i = 0; i < VARIANTS.length; i++) {
      ...
      if (rand < cumulative) {
        try { localStorage.setItem(STORAGE_KEY, VARIANTS[i]); } catch (e) {}
        return VARIANTS[i];
      }
    }
  ...
  if (window.posthog && typeof posthog.onFeatureFlags === 'function') {
    posthog.onFeatureFlags(function () {
      var variant = posthog.getFeatureFlag(FLAG_NAME);
      if (variant) {
        try { localStorage.setItem(STORAGE_KEY, variant); } catch (e) {}
        applyVariant(variant, 'posthog');
      }
    });
  }
  ...
  onReady(function () {
    if (!applied) {
      applyVariant(getLocalVariant(), 'local');
    }
  });
})();
